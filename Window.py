...
class Window(wx.Frame):
    ...
    def ToolBar(self):
        ...
        self.newtool = self.toolbar.AddTool(wx.ID_ANY, 'New', wx.Bitmap('icons/new.png'))
        self.opentool = self.toolbar.AddTool(wx.ID_ANY, 'Open', wx.Bitmap('icons/open.png'))
        self.savetool = self.toolbar.AddTool(wx.ID_ANY, 'Save', wx.Bitmap('icons/save.png'))
        self.saveastool = self.toolbar.AddTool(wx.ID_ANY, 'Save As', wx.Bitmap('icons/save-as.png'))
        self.runtool = self.toolbar.AddTool(wx.ID_ANY, 'Run Code', wx.Bitmap('icons/run.png'))
        self.toolbar.AddSeparator()
        self.undotool = self.toolbar.AddTool(wx.ID_ANY, 'Undo', wx.Bitmap('icons/undo.png'))
        self.redotool = self.toolbar.AddTool(wx.ID_ANY, 'Redo', wx.Bitmap('icons/redo.png'))
        self.toolbar.AddSeparator()
        self.cuttool = self.toolbar.AddTool(wx.ID_ANY, 'Cut', wx.Bitmap('icons/cut.png'))
        self.copytool = self.toolbar.AddTool(wx.ID_ANY, 'Copy', wx.Bitmap('icons/copy.png'))
        self.pastetool = self.toolbar.AddTool(wx.ID_ANY, 'Paste', wx.Bitmap('icons/paste.png'))
        self.toolbar.AddSeparator()
        self.zoomintool = self.toolbar.AddTool(wx.ID_ANY, 'Zoom In', wx.Bitmap('icons/zoom-in.png'))
        self.zoomouttool = self.toolbar.AddTool(wx.ID_ANY, 'Zoom Out', wx.Bitmap('icons/zoom-out.png'))
        self.toolbar.AddSeparator()
        self.abouttool = self.toolbar.AddTool(wx.ID_ANY, 'About', wx.Bitmap('icons/info.png'))
        self.quittool = self.toolbar.AddTool(wx.ID_ANY, 'Quit', wx.Bitmap('icons/quit.png'))
    ...
    def FileMenu(self):
        ...
        self.readonly = self.filemenu.Append(wx.ID_ANY, "&Read Only\tCtrl+Shift+R", "Set document to read only",
                                             wx.ITEM_CHECK)
    ...
    def EditMenu(self):
        ...
        self.formatmenu.AppendSeparator()
        self.wordwrap = self.formatmenu.Append(wx.ID_ANY, "&Word Wrap\tCtrl+W", "Set text to word wrapped",
                                               wx.ITEM_CHECK)
    ...
    def ViewMenu(self):
        ...
        self.normaltext = self.languagesmenu.Append(wx.ID_ANY, "&Normal Text\tAlt+Shift+0",
                                                    "Set syntax highlighting to Normal Text")
        self.languagesmenu.AppendSeparator()
        self.css = self.languagesmenu.Append(wx.ID_ANY, "&CSS\tAlt+Shift+2", "Set syntax highlighting to CSS")
        ...
        self.togalwaysontop = self.viewmenu.Append(wx.ID_ANY, "&Always on Top\tAlt+F1", "Enable/Disable always on top",
                                                   wx.ITEM_CHECK)
        self.fullscreen = self.viewmenu.Append(wx.ID_ANY, "&Fullscreen\tF11", "Fullscreen Codepad", wx.ITEM_CHECK)
        self.viewmenu.AppendSeparator()
        self.toglinenumbers = self.viewmenu.Append(wx.ID_ANY, "Toggle &Line Numbers\tAlt+Shift+L",
                                                   "Enable/Disable line numbers", wx.ITEM_CHECK)
        self.togstatusbar = self.viewmenu.Append(wx.ID_ANY, "Toggle &Status Bar\tAlt+Shift+S",
                                                 "Enable/Disable status bar", wx.ITEM_CHECK)
        self.togtransparent = self.viewmenu.Append(wx.ID_ANY, "Toggle &Transparent Mode\tAlt+Shift+T",
                                                   "Enable/Disable transparent mode", wx.ITEM_CHECK)
        self.togvieweol = self.viewmenu.Append(wx.ID_ANY, "Toggle &View Newline Endings\tAlt+Shift+E",
                                               "Enable/Disable view newline endings", wx.ITEM_CHECK)
        self.viewmenu.AppendSeparator()
        ...
        self.viewmenu.AppendSeparator()
        self.indentationguide = self.viewmenu.Append(wx.ID_ANY, "&View Indentation Guides\tCtrl+W",
                                                     "Enable/Disable indentation guide", wx.ITEM_CHECK)
        self.whitespace = self.viewmenu.Append(wx.ID_ANY, "&View Whitespace\tCtrl+Shift+W", "Enable/Disable whitespace",
                                               wx.ITEM_CHECK)

    def HelpMenu(self):
        # Help Menu
        self.helpmenu = wx.Menu()
        self.about = self.helpmenu.Append(wx.ID_ABOUT, "&About\tF1", "About Codepad")
        self.helpmenu.AppendSeparator()
        self.homepage = self.helpmenu.Append(wx.ID_ANY, "&Codepad Homepage\tCtrl+F2", "Codepad's homepage")
        self.github = self.helpmenu.Append(wx.ID_ANY, "&Codepad GitHub\tCtrl+F3", "Codepad's GitHub")
        self.helpmenu.AppendSeparator()
    ...
    def BindsMenu(self):
        ...
        self.Bind(wx.EVT_MENU, self.CSS, self.css)
    ...
    def New(self, e):
        if self.control.IsModified():
            dlg = wx.MessageBox('Would you like to save changes to ' + self.filename + '?', 'Save Changes?',
                                wx.YES_NO | wx.CANCEL)
            if dlg == wx.YES:
                self.Save(e)
                self.control.SetValue("")
                self.SetTitle("Untitled - Codepad")
        ...
        try:
            ...
            if (dlg.ShowModal() == wx.ID_OK):
                ...
                if self.suffix == "py":
                    ...
                    self.Python(e)
                elif self.suffix == "css":
                    self.CSS(e)
                else:
                    self.NormalText(e)
                f = open(os.path.join(self.dirname, self.filename), 'r')
                self.control.SetValue(f.read())
                self.SetTitle(self.filename + " - " + self.dirname + " - Codepad")
                f.close()
                self.control.SetModified(False)
            dlg.Destroy()
        except:
    ...
    def About(self, e):
        ...
        info.SetName('Codepad')
        info.SetIcon(wx.Icon('icons/favicon.png', wx.BITMAP_TYPE_PNG))
        info.SetVersion('BETA')
        info.SetCopyright('(C) 2019 Mohamed Altarawy')
        info.SetWebSite('http://altarawy.github.io/codepad/')
        wx.adv.AboutBox(info)

    def Homepage(self, e):
        webbrowser.open('https://altarawy.github.io/codepad/')

    def GitHub(self, e):
        webbrowser.open('https://github.com/altarawy/codepad/')

    def Version(self, e):
        webbrowser.open('https://github.com/altarawy/codepad/releases')
    ...
    def Run(self, e):
        ...
        lexer = monkeyLexer(input_stream)
    ...
    def CSS(self, e):
        ...
        self.control.StyleSetSpec(wx.stc.STC_CSS_ATTRIBUTE, 'fore:#005e5e')
        self.control.StyleSetSpec(wx.stc.STC_CSS_CLASS, 'fore:#005e5e')
        self.control.StyleSetSpec(wx.stc.STC_CSS_COMMENT, 'fore:#008000')
        self.control.StyleSetSpec(wx.stc.STC_CSS_DEFAULT, 'fore:#000000')
        self.control.StyleSetSpec(wx.stc.STC_CSS_DIRECTIVE, 'fore:#005e5e')
        self.control.StyleSetSpec(wx.stc.STC_CSS_DOUBLESTRING, 'fore:#800080')
        self.control.StyleSetSpec(wx.stc.STC_CSS_ID, 'fore:#008080')
        self.control.StyleSetSpec(wx.stc.STC_CSS_IDENTIFIER, 'fore:#000000')
        self.control.StyleSetSpec(wx.stc.STC_CSS_IDENTIFIER2, 'fore:#000000')
        self.control.StyleSetSpec(wx.stc.STC_CSS_IMPORTANT, 'fore:#000000')
        self.control.StyleSetSpec(wx.stc.STC_CSS_OPERATOR, 'fore:#800000')
        self.control.StyleSetSpec(wx.stc.STC_CSS_PSEUDOCLASS, 'fore:#008080')
        self.control.StyleSetSpec(wx.stc.STC_CSS_SINGLESTRING, 'fore:#800080')
        self.control.StyleSetSpec(wx.stc.STC_CSS_TAG, 'fore:#008080')
        self.control.StyleSetSpec(wx.stc.STC_CSS_UNKNOWN_IDENTIFIER, 'fore:#000000')
        self.control.StyleSetSpec(wx.stc.STC_CSS_UNKNOWN_PSEUDOCLASS, 'fore:#008080')
        self.control.StyleSetSpec(wx.stc.STC_CSS_VALUE, 'fore:#668B8B')
